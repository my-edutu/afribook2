    ...
    <div className="flex flex-col bg-slate-50 min-h-screen">
      ...
      <div className="px-6 pt-16 pb-12 bg-gradient-to-b from-sky-600 to-sky-800 text-white rounded-b-[4rem] shadow-2xl relative overflow-hidden">
        ...
        <div className="mt-10 p-6 bg-white/10 backdrop-blur-3xl border border-white/20 rounded-[32px] flex items-center gap-5 relative z-10 shadow-2xl">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-sky-200 mb-1.5">Memory match engine</p>
            <p className="text-sm font-black text-white tracking-tight leading-tight">3 potential classmates from nysc 2017 joined today.</p>
          </div>
        </div>
      </div>
      ...
      <main className="flex-1 px-6 pt-12 pb-32">
        ...
        <div className="mb-12">
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full h-16 pl-14 pr-7 bg-white border border-slate-100 rounded-[28px] shadow-sm focus:outline-none focus:ring-4 focus:ring-sky-500/5 transition-all font-bold text-slate-900"
            />
          </div>
        </div>
        ...
        <section className="mb-12">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-black text-slate-900 tracking-tight leading-none">Geomemory map</h2>
            <button className="text-sky-600 text-[10px] font-black uppercase tracking-widest bg-sky-50 px-4 py-2 rounded-xl border border-sky-100">Live map</button>
          </div>
          <div className="relative h-56 rounded-[48px] bg-slate-200 overflow-hidden border-4 border-white shadow-xl shadow-slate-200/50">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-white/90 backdrop-blur-md px-6 py-3 rounded-full border border-white flex items-center gap-3 shadow-2xl">
                ...
                <span className="text-xs font-black text-slate-900 uppercase tracking-tight">12 memories near lagos</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mb-12">
          <div className="flex justify-between items-end mb-8">
            <h2 className="text-2xl font-black text-slate-900 leading-none tracking-tight">Your circles</h2>
            <button className="text-sky-600 text-[10px] font-black uppercase tracking-widest bg-sky-50 px-4 py-2 rounded-xl border border-sky-100">Join new</button>
          </div>
          <div className="space-y-5">
            {memoryCircles.map((circle, i) => (
              <div key={i} className="bg-white p-5 rounded-[40px] border border-slate-50 shadow-sm flex items-center gap-5 group cursor-pointer active:scale-[0.98] transition-all hover:shadow-md">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h4 className="font-black text-slate-900 text-lg leading-tight mb-1 truncate lowercase first-letter:uppercase">{circle.title}</h4>
        ...
        <section className="mb-12 p-10 bg-sky-50 border border-sky-100 rounded-[56px] relative overflow-hidden shadow-sm">
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-6 text-sky-600">
              <Sparkles size={28} />
              <h3 className="font-black uppercase tracking-[0.2em] text-[10px]">Strategic match</h3>
            </div>
            <p className="text-xl font-black text-sky-950 leading-tight mb-8">
              "We noticed you and chioma lived in surulere from 2009–2012."
            </p>
            <div className="flex gap-4">
              <button className="bg-sky-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-sky-600/20 active:scale-95 transition-all">Recall together</button>
              <button className="bg-white text-sky-600 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest border border-sky-100 active:scale-95 transition-all">Dismiss</button>
            </div>
          </div>
